...
import numpy as np
import onepose
from extract_dominant_colors import get_dominant_colors, calculate_color_match_score
...
def infer_on_dataset(
    ...
    perspective_matrix,
    goal_realworld_size,
    goalkeeper_clothes_colors,  # If not None, we pick best skeleton among persons
    save_img,
    save_txt,
    save_conf,
    save_crop,
    ...
    """
    Main inference loop.
    If goalkeeper_clothes_colors is a list of LAB colors, 
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):
            ...
            # --------------------------------------
            # 4) If goalkeeper_clothes_colors is set, 
            #    keep only the best skeleton among persons
            # --------------------------------------
            if goalkeeper_clothes_colors is not None:
...
def process_single_detection(
    ...
    if c == 0:
        ...
        if keypoints_dict and warped_points is not None:
            ...
            if clothes_colors is not None:
                # Example logic to compute color match for the torso region
                if (keypoints_dict['confidence'][5] > 0.5 and
                    keypoints_dict['confidence'][6] > 0.5 and
                    keypoints_dict['confidence'][11] > 0.5 and
                    keypoints_dict['confidence'][12] > 0.5):
                    
                    # Shoulders & hips
                    x_min = int(min(
                        keypoints_dict['points'][5][0],
                        keypoints_dict['points'][6][0],
                        keypoints_dict['points'][11][0],
                        keypoints_dict['points'][12][0]
                    ))
                    x_max = int(max(
                        keypoints_dict['points'][5][0],
                        keypoints_dict['points'][6][0],
                    ...
                    y_min = int(min(
                        keypoints_dict['points'][5][1],
                        keypoints_dict['points'][6][1]
                    ))
                    y_max = int(max(
                        keypoints_dict['points'][11][1],
                        keypoints_dict['points'][12][1]
                    ))

                    # clamp
                    x_min = max(0, x_min)
                    x_max = min(im0s.shape[1], x_max)
                    y_min = max(0, y_min)
                    y_max = min(im0s.shape[0], y_max)

                    if x_min < x_max and y_min < y_max:
                        torso_crop = im0s[y_min:y_max, x_min:x_max]
                        # get dominant colors
                        torso_colors, torso_colors_area = get_dominant_colors(torso_crop, k=3)
                        # compute match score
                        score_val = calculate_color_match_score(
                            clothes_colors,
                            torso_colors,
                            torso_colors_area,
                            threshold=50
...
def run(
    ...
    goalkeeper_clothes_colors=None,    # list of LAB colors for color matching
):
    """
...
def parse_opt():
    ...
    parser.add_argument('--goalkeeper_clothes_colors', type=int, default=[[[0, 128, 128]]], help='list of LAB colors for color matching')
