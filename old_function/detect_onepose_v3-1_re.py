...
def infer_on_dataset(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):
            ...
            # -------------
            # (A) Save all detections to YOLO TXT here
            # -------------
            if save_txt and goal_realworld_size is not None and len(all_detections_skeletons_for_frame) > 0:
                # pass in entire detection list
            ...
            # -------------
            # (B) Draw bounding boxes & skeletons in one place
            # -------------
            draw_all_detections(
                im0_warped,
                all_detections_skeletons_for_frame,
...
def process_single_detection(
    ...
    """
    Return detection data (bbox, skeleton, label, etc.) but do NOT
    perform any drawing or text saving here.
    """
    ...
    if have_perspective and perspective_matrix is not None:
        ...
        # No perspective warp, same coords
        detection_result['bbox_warp'] = [x1, y1, x2, y2]

    # Pose estimation if c==0 => "person"
    if c == 0:
