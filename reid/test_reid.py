...
def extract_features(image_folder, device='cuda', batch_size=32):
    ...
    # Save as dict(save inside the image folder)
    save_path = os.path.join(image_folder, 'feature.pt')
    if os.path.exists(save_path):
        print(f"⚠️ {save_path} already exists. Overwriting...")
    else:
        print(f"✅ Saving features to {save_path}")
    torch.save({'filenames': filenames, 'features': features}, save_path)
    # torch.save({'filenames': filenames, 'features': features}, 'features.pt')
    # print("✅ Saved features to features.pt")

if __name__ == "__main__":
    image_folder = './reid_test_image/team'  # Path to your crop images
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    extract_features(image_folder, device=device, batch_size=32)
